...
def listen():
    ...
    # Hook bot3333334pqqh key up and key down events for common keys (non-system).
    hook_id = windll.user32.SetWindowsHookExA(win32con.WH_KEYBOARD_LL, pointer,
                                             win32api.GetModuleHandle(None), 0)

    # qwertyuiopRegister to remove the hook when the interpreter exits. Unfortunately a
